...
contains
  ...
  function compute_dlml(l,m,sincos,cos2) result(dlml)
    ...
    do i=0,l-1       
       if ((ll<l) .AND. (l-m+mm-ll>0)) then
          ...
          dlml = dlml * SQRT(real((2_dp*ll+2_dp)*(2_dp*ll+3_dp),kind=dp)/real((ll+mm+1_dp)*(ll-mm+1_dp),kind=dp))*sincos
          ll = ll + 1_dp
       end if
       if ((mm<m) .AND. (ll<l)) then
          ! l,m -> l+1 m+1
          dlml = dlml * SQRT(real((2_dp*ll+2_dp)*(2_dp*ll+3_dp),kind=dp)/real((ll+mm+1_dp)*(ll+mm+2_dp),kind=dp))*cos2
          ll = ll + 1_dp
          mm = mm + 1_dp 
       end if
  ...
  subroutine dlml_recursion_l_contiguous(dlml,m,bw,sincos,cos2)
    ...
    do i=1,min(bw-m,swap_id)
       ...
       dlml(:,i) = MERGE(dlml(:,i), dlml(:,i) * SQRT(real((2_dp*l+2_dp)*(2_dp*l+3_dp),kind=dp)/real((l+m)*(l+m+1_dp),kind=dp))*cos2,m==0)
    end do
    do i=swap_id,bw-m-1
       !(l-> l+1)
       l=m+(i-1_dp)
       dlml(:,i+1) = dlml(:,i) * SQRT(real((2_dp*l+2_dp)*(2_dp*l+3_dp),kind=dp)/real((l+m+1_dp)*(l-m+1_dp),kind=dp))*sincos
    end do
  end subroutine dlml_recursion_l_contiguous
  function compute_all_dlml_l_contiguous(bw,sincos,cos2) result(dlml)
    ...
    ! Setup workspace
    dlml_tmp(:,1) = SQRT(0.5_dp)
    
    ! Do stair recursion alternating between (l -> l+1) and (l,m -> l+1,m+1 ) steps.
    do m=0,bw-1
       call dlml_recursion_l_contiguous(dlml_tmp, m, bw, sincos, cos2)
  ...
  subroutine dlml_recursion_m_contiguous(dlml,l,bw,sincos,cos2)
    !! dlml for all l at fixed m. Has to be of size [Size(sincos,1),bw]
    real(kind = dp),intent(inout) :: dlml(:,:)
    !! bandwidth 0<=l<bw
    ...
    !! Arrays containing $sin(\frac{\beta}{2})*cos(\frac{\beta}{2})$ and $cos(\frac{\beta}{2})**2$
    real(kind = dp),intent(in) :: sincos(:),cos2(:)
    integer(kind = dp) :: i,m,swap_id

    swap_id = l/2
    do m=l,swap_id,-1
       !(l,m -> l+1,m+1 )
       dlml(:,m+2) = dlml(:,m+1) * SQRT(real((2_dp*l+2_dp)*(2_dp*l+3_dp),kind=dp)/real((l+m+1_dp)*(l+m+2_dp),kind=dp))*cos2
    end do
    do m=0,swap_id
       !(l-> l+1)
       dlml(:,m+1) = dlml(:,m+1) * SQRT(real((2_dp*l+2_dp)*(2_dp*l+3_dp),kind=dp)/real((l+m+1_dp)*(l-m+1_dp),kind=dp))*sincos
    end do
    
  end subroutine dlml_recursion_m_contiguous
  function compute_all_dlml_m_contiguous(bw,sincos,cos2) result(dlml)
    ...
    ! Do stair recursion alternating between (l -> l+1) and (l,m -> l+1,m+1 ) steps.
    do l=0,bw-2
       call dlml_recursion_m_contiguous(dlml_tmp, l, bw, sincos, cos2)
       lm_id = triangular_to_flat_index_reversed(l+1,0_dp,bw)
       next_lm_id = triangular_to_flat_index_reversed(l+1,l+1,bw)
  ...
  function genWig_L2(m1,m2,bw,cos,dlml) result(wigners_m1m2)
    ...
    workspace(:,1) = 0
    workspace(:,2) = dlml !compute_dlml(l_start,m1,trig_samples(:,3),trig_samples(:,2))
    ...
    do i=1, bw-l_start-1
       ...
       workspace(:,5) = workspace(:,4) * cos !trig_samples(:,1)
  ...
  function wigner_slice_trsp(m1,m2,bw) result(slice)
  ...
  end function wigner_slice_trsp
  ...
  function genWigAll(bw) result(wigners)
    ...
    do m1=0,bw-1
       call dlml_recursion_l_contiguous(dlml_workspace, m1, bw, trig_samples(:,2), trig_samples(:,3))
       do m2=m1, bw-1
...
module so3ft
  ...
  real(kind = dp), allocatable, target :: wigner_dlml(:,:)
  ...
  abstract interface
     subroutine inverse_wigner_interface(coeff,so3func,m1,m2,sym_array,sym_const_m1)
     ...
     subroutine forward_wigner_interface(so3func,coeff,m1,m2,sym_array,sym_const_m1)
       import :: dp  ! Otherwise dp is undefined in _abstract interface blocks
       complex(kind = dp), intent(inout) :: coeff(:)
     ...
     subroutine wigner_corr_interface(f_ml,g_ml,so3func,m1,m2,sym_array,wig_norm,sym_const_m1,pm1_slice,nm1_slice)
       import :: dp  ! Otherwise dp is undefined in _abstract interface blocks
     ...
     subroutine wigner_corr_real_interface(f_ml,g_ml,so3func,m1,m2,sym_array,wig_norm,sym_const_m1,pm1_slice)
       import :: dp  ! Otherwise dp is undefined in _abstract interface blocks
       complex(kind = dp), intent(in) :: f_ml(:),g_ml(:)
       complex(kind = dp), intent(inout) :: so3func(:,:,:)
...
contains
  ...
  subroutine init_wigners()
    integer(kind = dp) :: m1, m2, d_shape(2),l_slice(2),trsp_slice(2),wig_len,lid
    d_shape = wigner_d_shape(bw)
    allocate(wigner_d(d_shape(1),d_shape(2)))
    call genWigAll_preallocated(bw,wigner_d)
    
    allocate(wigner_d_trsp(d_shape(1)*d_shape(2)))
    
    do m1=0,bw-1
       do m2=m1, bw-1
          l_slice = wigner_l_slice(m1,m2,bw)
          trsp_slice = wigner_slice_trsp(m1,m2,bw)
          wig_len = 2*bw*(bw-max(abs(m1),abs(m2)))          
          wigner_d_trsp(trsp_slice(1):trsp_slice(2)) = reshape(transpose(wigner_d(:,l_slice(1):l_slice(2))),[wig_len])
          ! multiply untransposed wigners with legendre weights
          do lid=l_slice(1), l_slice(2)
             wigner_d(:,lid) = wigner_d(:,lid)*legendre_weights
          end do
       end do
    end do
  ...
  subroutine init(bandwidth,l_max,n_threads,precompute_wigners,init_ffts,fftw_flag)
    ...
    if ( (bandwidth/=bw) .OR. (.NOT. precompute_wigners)) then
       ...
       if (precompute_wigners) then
          ...
          allocate(wigner_dlml(2*bw,triangular_size(bw)))
          wigner_dlml = compute_all_dlml_l_contiguous(bw, trig_samples(:,2), trig_samples(:,3))
       end if
    else
  ...
  subroutine destroy()
    ...
    if (allocated(wigner_dlml)) then
       deallocate(wigner_dlml)
    end if
  ...
  function get_wigner_matrix_copy(m1,m2,bw) result(wig_mat)
    ...
    integer(kind = dp) dlml_id,slice(2)
    ...
    if (allocated(wigner_d)) then
       ...
       wig_mat = genWig_L2(m1,m2,bw,trig_samples(:,1),wigner_dlml(:,dlml_id))
    end if
  ...
  subroutine inverse_wigner_loop_body_cmplx_alloc(coeff,so3func,m1,m2,sym_array,sym_const_m1)
    complex(kind = dp), intent(in) :: coeff(:)
    complex(kind = dp), intent(inout) :: so3func(:,:,:)
    integer(kind=dp), intent(in) :: m1,m2
    real(kind=dp),intent(in) :: sym_array(:),sym_const_m1
    real(kind = dp),pointer :: wig_mat(:,:)
    complex(kind = dp) :: coeff_part(bw-m2)
    real(kind = dp) :: sym_const_m2
    integer(kind=dp) :: s_ids(2),c_slice(2),bw2,t_slice(2)
    ...
    ! get wigner matrix
    t_slice = wigner_slice_trsp(m1,m2,bw)
    wig_mat(1:bw-m2,1:2*bw) => wigner_d_trsp(t_slice(1):t_slice(2))
  ...
  end subroutine inverse_wigner_loop_body_cmplx_alloc
  subroutine inverse_wigner_loop_body_cmplx(coeff,so3func,m1,m2,sym_array,sym_const_m1)
    complex(kind = dp), intent(in) :: coeff(:)
    complex(kind = dp), intent(inout) :: so3func(:,:,:)
    integer(kind=dp), intent(in) :: m1,m2
    real(kind=dp),intent(in) :: sym_array(:),sym_const_m1
    real(kind = dp) :: wig_mat(bw-m2,2*bw)
    complex(kind = dp) :: coeff_part(bw-m2)
    real(kind = dp) :: sym_const_m2
    integer(kind=dp) :: s_ids(2),c_slice(2),bw2,dlml_id
    ...
    ! get wigner matrix
    dlml_id = triangular_to_flat_index(m1,m2,bw)
    wig_mat = TRANSPOSE(genWig_L2(m1,m2,bw,trig_samples(:,1),wigner_dlml(:,dlml_id)))
  ...
  subroutine inverse_wigner_trf_cmplx(coeff,so3func,use_mp)
    ...
    real(kind=dp) :: sym_const_m1,sym_array(bw)
    procedure(inverse_wigner_interface),pointer :: loop_body

    ! Select loop_body
    if (allocated(wigner_d_trsp)) then
       loop_body => inverse_wigner_loop_body_cmplx_alloc
    else
       loop_body => inverse_wigner_loop_body_cmplx
       end if
    ...
    if (use_mp) then
       ...
       do i=1,(L+1)*(L+2)/2
          ...
          !print *,i,m1,m2
          sym_const_m1 = (-1.0)**m1
          call loop_body(coeff,so3func,m1,m2,sym_array,sym_const_m1)
       end do
       !$OMP END DO
       !$OMP END PARALLEL 
    else
  ...
  subroutine forward_wigner_loop_body_cmplx_alloc(so3func,coeff,m1,m2,sym_array,sym_const_m1)
    complex(kind = dp), intent(inout) :: coeff(:)
    complex(kind = dp), intent(in) :: so3func(:,:,:)
    integer(kind=dp), intent(in) :: m1,m2
    real(kind=dp),intent(in) :: sym_array(:),sym_const_m1
    real(kind = dp),pointer :: wig_mat(:,:)
    real(kind = dp) :: sym_const_m2     
    integer(kind=dp) :: s_ids(2),c_slice(2),bw2,l_slice(2)
    ...
    l_slice = wigner_l_slice(m1, m2, bw)
    wig_mat(1:bw2,1:bw-m2) => wigner_d(:,l_slice(1):l_slice(2))
    ...
    so3func_part = so3func(bw2:1:-1,s_ids(1),s_ids(2))*sym_const_m2
    coeff(c_slice(1):c_slice(2)) = matmul(so3func_part,wig_mat)*sym_array(m2+1:)
  end subroutine forward_wigner_loop_body_cmplx_alloc
  subroutine forward_wigner_loop_body_cmplx(so3func,coeff,m1,m2,sym_array,sym_const_m1)
    ...
    real(kind=dp),intent(in) :: sym_array(:),sym_const_m1
    real(kind = dp) :: sym_const_m2     
    real(kind = dp) :: wig_mat(2*bw,bw-m2) ! possible source of optimization can make it allocatable
    integer(kind=dp) :: s_ids(2),c_slice(2),bw2,dlml_id
  ...
  subroutine forward_wigner_trf_cmplx(so3func,coeff,use_mp)
    ...
    ! Select loop_body
    if (allocated(wigner_d)) then
       loop_body => forward_wigner_loop_body_cmplx_alloc
    else
       loop_body => forward_wigner_loop_body_cmplx
    end if
    ...
    if (use_mp) then
       ...
       do i=1,(L+1)*(L+2)/2
          call flat_to_triangular_index(m1,m2,i,L)
          sym_const_m1 = (-1.0)**m1
          call loop_body(so3func,coeff,m1,m2,sym_array,sym_const_m1)
       end do
       !$OMP END DO
       !$OMP END PARALLEL 
    else
  ...
  subroutine inverse_wigner_loop_body_real_alloc(coeff,so3func,m1,m2,sym_array,sym_const_m1)
    ...
    integer(kind=dp) :: s_ids(2),c_pm1pm2_slice(2),c_nm2nm1_slice(2),c_pm1nm2_slice(2),c_pm2nm1_slice(2),bw2,t_slice(2)
  ...
  end subroutine inverse_wigner_loop_body_real_alloc
  subroutine inverse_wigner_loop_body_real(coeff,so3func,m1,m2,sym_array,sym_const_m1)
    ...
    real(kind = dp) :: wig_mat(bw-m2,2*bw)
    complex(kind = dp):: coeff_part(bw-m2)
    integer(kind=dp) :: s_ids(2),c_pm1pm2_slice(2),c_nm2nm1_slice(2),c_pm1nm2_slice(2),c_pm2nm1_slice(2),bw2,dlml_id
  ...
  subroutine inverse_wigner_trf_real(coeff,so3func,use_mp)
    ...
    complex(kind = dp), intent(in) :: coeff(:)
    logical, intent(in) :: use_mp
    integer(kind = dp) :: i,m1,m2,L,s_ids(2),s_ids_sym(2)
    real(kind=dp) :: sym_const_m1,sym_array(bw)
    procedure(inverse_wigner_interface),pointer :: loop_body

    ! Select loop_body
    if (allocated(wigner_d_trsp)) then
       loop_body => inverse_wigner_loop_body_real_alloc
    else
       loop_body => inverse_wigner_loop_body_real
    end if
  ...
  subroutine forward_wigner_loop_body_real_alloc(so3func,coeff,m1,m2,sym_array,sym_const_m1)
    ! This subroutine assumes 0<=m1<=m2<=bw
    ! which also means m = max(abs(m1),abs(m2)) = m2
    complex(kind = dp), intent(in) :: so3func(:,:,:)
    complex(kind = dp), intent(inout) :: coeff(:)
    ...
    complex(kind = dp) :: so3func_part(2*bw)
    integer(kind=dp) :: s_ids(2),c_slice(2),c_pm1pm2_slice(2),c_nm2nm1_slice(2),c_pm1nm2_slice(2),c_pm2nm1_slice(2),bw2,l_slice(2)
  ...
  end subroutine forward_wigner_loop_body_real_alloc
  subroutine forward_wigner_loop_body_real(so3func,coeff,m1,m2,sym_array,sym_const_m1)
    ...
    real(kind=dp),intent(in) :: sym_array(:),sym_const_m1
    real(kind = dp) :: sym_const_m2
    real(kind = dp) :: wig_mat(2*bw,bw-m2)
    complex(kind = dp) :: so3func_part(2*bw)
    integer(kind=dp) :: s_ids(2),c_slice(2),c_pm1pm2_slice(2),c_nm2nm1_slice(2),c_pm1nm2_slice(2),c_pm2nm1_slice(2),bw2,dlml_id
    ...
    c_pm1nm2_slice = coeff_slice(m1,-m2,bw)
    so3func_part = so3func(bw2:1:-1,s_ids(1),s_ids(2))*sym_const_m1
    coeff(c_pm1nm2_slice(1):c_pm1nm2_slice(2)) = matmul(legendre_weights*so3func_part,wig_mat)*sym_array(m2+1:)
    ...
    c_pm2nm1_slice = coeff_slice(m2,-m1,bw)
    so3func_part = so3func(bw2:1:-1,s_ids(1),s_ids(2))*sym_const_m1
    coeff(c_pm2nm1_slice(1):c_pm2nm1_slice(2)) = matmul(legendre_weights*so3func_part,wig_mat)*sym_array(m2+1:)
  ...
  subroutine forward_wigner_trf_real(so3func,coeff,use_mp)
    ...
    ! Select loop_body
    if (allocated(wigner_d)) then
       loop_body => forward_wigner_loop_body_real_alloc
    else
       loop_body => forward_wigner_loop_body_real
    end if
  ...
  subroutine inverse_wigner_loop_body_corr_cmplx_alloc(f_ml,g_ml,so3func,m1,m2,sym_array,wig_norm,sym_const_m1,pm1_slice,nm1_slice)
    complex(kind = dp), intent(in) :: f_ml(:),g_ml(:)
    complex(kind = dp), intent(inout) :: so3func(:,:,:)
    integer(kind=dp), intent(in) :: m1,m2,pm1_slice(:),nm1_slice(:)
    real(kind=dp),intent(in) :: sym_array(:),wig_norm(:),sym_const_m1
    ...
    integer(kind=dp) :: s_ids(2),bw2,nm2_slice(2),pm2_slice(2),l_start,t_slice(2)
    ...
    cc_lmn = wig_norm(l_start:) * f_ml(nm2_slice(1):nm2_slice(2)) * g_ml(pm1_slice(1):pm1_slice(2)) * sym_const_m1m2 &
         & * sym_array(l_start:)
    so3func(bw2:1:-1, s_ids(1),s_ids(2)) = sym_const_m2*matmul(cc_lmn,wig_mat)
  end subroutine inverse_wigner_loop_body_corr_cmplx_alloc
  subroutine inverse_wigner_loop_body_corr_cmplx(f_ml,g_ml,so3func,m1,m2,sym_array,wig_norm,sym_const_m1,pm1_slice,nm1_slice)
    ...
    real(kind = dp) :: sym_const_m2,sym_const_m1m2
    complex(kind = dp) :: cc_lmn(bw-m2)
    integer(kind=dp) :: s_ids(2),bw2,nm2_slice(2),pm2_slice(2),l_start,dlml_id
  ...
  subroutine cross_correlation_ylm_cmplx(f_lm,g_lm,cc,use_mp)
    ...
    integer(kind = dp) :: i,m1,m2,m,L,pm1_slice(2),nm1_slice(2)    
    real(kind=dp) :: sym_const_m1,sym_array(bw),wig_norm(bw)
    procedure(wigner_corr_interface),pointer :: loop_body

    if (allocated(wigner_d_trsp)) then
       loop_body => inverse_wigner_loop_body_corr_cmplx_alloc
    else
       loop_body => inverse_wigner_loop_body_corr_cmplx
    end if
    ...
    if (use_mp) then
       ...
       do i=1,(L+1)*(L+2)/2
          ...
          call loop_body(f_ml,g_ml,fft_c2c_in,m1,m2,sym_array,wig_norm,sym_const_m1,pm1_slice,nm1_slice)
       end do
       !$OMP END DO
       !$OMP END PARALLEL
    else
  ...
  subroutine inverse_wigner_loop_body_corr_real_alloc(f_ml,g_ml,so3func,m1,m2,sym_array,wig_norm,sym_const_m1,pm1_slice)
    ! This subroutine assumes 0<=m1<=m2<=bw
    ! which also means m = max(abs(m1),abs(m2)) = m2
    complex(kind = dp), intent(in) :: f_ml(:),g_ml(:)
    complex(kind = dp), intent(inout) :: so3func(:,:,:)
    ...
    real(kind = dp) :: sym_const_m2,sym_const_m1m2
    complex(kind = dp) :: cc_lmn(bw-m2)
    integer(kind=dp) :: s_ids(2),bw2,pm2_slice(2),l_start,t_slice(2)
    ...
    cc_lmn = wig_norm(l_start:) * f_ml(pm2_slice(1):pm2_slice(2)) * CONJG(g_ml(pm1_slice(1):pm1_slice(2))) * sym_const_m2 &
         & * sym_array(l_start:)
    so3func(bw2:1:-1, s_ids(1),s_ids(2)) = sym_const_m1*matmul(cc_lmn,wig_mat)    
  end subroutine inverse_wigner_loop_body_corr_real_alloc
  subroutine inverse_wigner_loop_body_corr_real(f_ml,g_ml,so3func,m1,m2,sym_array,wig_norm,sym_const_m1,pm1_slice)
    ...
    real(kind = dp) :: sym_const_m2,sym_const_m1m2
    complex(kind = dp) :: cc_lmn(bw-m2)
    integer(kind=dp) :: s_ids(2),bw2,pm2_slice(2),l_start,dlml_id
  ...
  subroutine cross_correlation_ylm_real(f_ml,g_ml,cc,use_mp)
    ! let f,g be two square integrable functions on the 2 sphere 
    ! Define CC: SO(3) ---> \mathbb{R},   R |---> <f,g \circ R> = \int_{S^2} dx f(x)*\overline{g(Rx)}
    ! This function calculates CC(R) for all R defined in make_SO3_grid
    ! Spherical harmonic coefficient arrays , g_lm,f_lm, are assumed to follow the indexing conventions of the SHTNS package,
    ...
    logical, intent(in) :: use_mp
    integer(kind = dp) :: i,m,m1,m2,L,pm1_slice(2),s_ids(2),s_ids_sym(2)
    real(kind=dp) :: sym_const_m1,sym_array(bw),wig_norm(bw)
    procedure(wigner_corr_real_interface),pointer :: loop_body

    if (allocated(wigner_d_trsp)) then
       loop_body => inverse_wigner_loop_body_corr_real_alloc
    else
       loop_body => inverse_wigner_loop_body_corr_real
    end if
    ...
    fft_c2r_in = 0._dp
       ...
       !$OMP PARALLEL PRIVATE(i,l,m,m1,m2,sym_const_m1,pm1_slice) SHARED(f_ml,g_ml,fft_c2c_in,sym_array,wig_norm)
       ...
       do i=1,(L+1)*(L+2)/2
          ...
          sym_const_m1 = (-1.0)**m1
          pm1_slice= MLc_slice(m1,bw)
          call loop_body(f_ml,g_ml,fft_c2c_in,m1,m2,sym_array,wig_norm,sym_const_m1,pm1_slice)
       end do
       !$OMP END DO
       ...
       do m1=0, L
          ...
          pm1_slice= MLc_slice(m1,bw)
          do m2=m1, L          
             call loop_body(f_ml,g_ml,fft_c2r_in,m1,m2,sym_array,wig_norm,sym_const_m1,pm1_slice)
          end do
       end do
  ...
  subroutine corss_correlation_ylm_3D_real(f_lms,g_lms,cc,radial_sampling_points,radial_limits,nthreads)
    ...
    if (nthreads==1_dp) then
       do rid=radial_limits(1),radial_limits(2)
          call cross_correlation_ylm_real(f_lms(:,rid),g_lms(:,rid),fft_r2c_out,.FALSE.)
