...
from pysofft.old.make_wiegner import SinEvalPts,SinEvalPts2,CosEvalPts,CosEvalPts2,n_wigners
from pysofft.old.make_wiegner import genWigAll,genWigAllTrans,get_euler_angles,wig_little_d_grid,wig_little_d_lookup
from pysofft.old.wignerTransform import wigNaiveAnalysis_fftw,wigNaiveSynthesis_fftw
from pysofft.old.wignerTransform import wigNaiveAnalysis_fftwX,wigNaiveSynthesis_fftwX
from pysofft.old.wignerTransform import wigNaiveAnalysis_fftwY,wigNaiveSynthesis_fftwY
from pysofft.old.wignerWeights import makeweights2
from pysofft.old.rotate import rotate_coeff_multi,rotate_coeff
...
def d_to_coeff_info_v2(bw):
    ...
    ids = np.zeros((totalCoeffs_so3(bw),3),dtype = np.int32)
    lnk_to_ids = wig_little_d_lookup(bw)
    lookup = {l:np.zeros((2*l+1,2*l+1,3),int) for l in range(bw)}
    start = time.time()
    _nk_range = np.roll(np.arange(-bw+1,bw),bw)
    pos = 0
    for n in _nk_range:
